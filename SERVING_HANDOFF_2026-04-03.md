...
The production stack uses vLLM for T3 decode and the **turbo S3** checkpoint for mel synthesis. The turbo S3 uses a 2-step meanflow ODE (~5× faster than the default 10-step S3Gen). This checkpoint is NOT included in the repo — it must be present on disk.

**HF repo:** `ResembleAI/chatterbox-turbo`

**Required file:** `s3gen_meanflow.safetensors`

**Current cached path on this machine:**
```
~/.cache/huggingface/hub/models--ResembleAI--chatterbox-turbo/snapshots/749d1c1a46eb10492095d68fbcf55691ccf137cd/
```

**If the HF cache is missing on a fresh machine**, download explicitly:
...
mkdir -p "$TURBO_S3_CHECKPOINT_DIR"
...
snapshot_download(
    ...
    local_dir=os.environ["TURBO_S3_CHECKPOINT_DIR"],
    local_dir_use_symlinks=False,
    token=os.environ.get("HF_TOKEN"),
)
print("turbo S3 checkpoint ready:", os.environ["TURBO_S3_CHECKPOINT_DIR"])
PY
```

**If `TURBO_S3_CHECKPOINT_DIR` is not set:** the service auto-downloads from HF at startup. This requires `HF_TOKEN` and network access. Set it explicitly to bypass auto-download.

**CRITICAL backup note:** Before decommissioning this machine, copy the turbo S3 snapshot directory to persistent storage. If the HF repo becomes inaccessible, this is the only recovery path.
...
## Pre-Decommission Audit Findings (2026-04-03)

Two code bugs found and fixed in this session:

1. **`VLLM_ENFORCE_EAGER` default was inverted** (`fastapi_vllm_tts_service.py:727`)
   - Was: `_env_bool("VLLM_ENFORCE_EAGER", True)` — defaulted to eager mode (slower, ~3s first_chunk_s)
   - Fixed to: `_env_bool("VLLM_ENFORCE_EAGER", False)` — defaults to CUDA graphs (~47% faster)
   - This means starting the service without setting `VLLM_ENFORCE_EAGER=false` now correctly uses CUDA graphs by default

2. **No guard against `VLLM_ENABLE_PREFIX_CACHING=true`** (`fastapi_vllm_tts_service.py:_load_service_model`)
   - Added early `RuntimeError` that rejects startup if prefix caching is enabled
   - Prevents silent quality regression (batch rows 1..N producing noisy tails)

Three documentation gaps found (not code bugs):

3. **Turbo S3 download command was missing from docs** — now added above in "Turbo S3 Checkpoint" section
4. **No explicit backup command for turbo S3 checkpoint** — see backup checklist below
5. **`SPK_17_000003.wav` must be present at repo root** — it is tracked in the repo, will be present after a fresh clone

## Minimum Backup Checklist Before GPU Loss

Before taking this box offline, back up these artifacts to persistent storage:

```bash
# Turbo S3 checkpoint (most critical — not in repo)
cp -r ~/.cache/huggingface/hub/models--ResembleAI--chatterbox-turbo/ /your/backup/location/

# Base T3 multilingual weights (can be re-downloaded from HF but slow)
cp -r ~/.cache/huggingface/hub/models--ResembleAI--chatterbox/ /your/backup/location/

# vLLM T3 export package (can be regenerated but saves 10 min on next setup)
cp -r /home/ubuntu/ChatterBox_S3_Concurrency/runs/t3_vllm_export/ /your/backup/location/

# Conda env spec (saves full rebuild time)
conda activate chatterbox-vllm
conda env export > /your/backup/location/chatterbox_vllm_env.yml
```

The repo itself (both parent and submodule) is already pushed to GitHub — code is safe.

## Model Artifact Chain Summary

| Artifact | Source | Required? | Cache Path |
|---|---|---|---|
| Base T3 weights (`t3_mtl23ls_v2.safetensors`) | `ResembleAI/chatterbox` on HF | Yes | `~/.cache/huggingface/hub/models--ResembleAI--chatterbox/snapshots/05e904af2b5c7f8e482687a9d7336c5c824467d9/` |
| vLLM T3 export package | Generated by `export_vllm_t3_model.py` | Yes | `runs/t3_vllm_export/` |
| Turbo S3 checkpoint (`s3gen_meanflow.safetensors`) | `ResembleAI/chatterbox-turbo` on HF | Yes | `~/.cache/huggingface/hub/models--ResembleAI--chatterbox-turbo/snapshots/749d1c1a46eb10492095d68fbcf55691ccf137cd/` |
| Audio prompt WAV | In repo | Yes | `SPK_17_000003.wav` at repo root |
