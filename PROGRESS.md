...
- **captured live token-to-mel / HiFT geometry and refreshed the serving handoff docs:**
  ...
  - added a first-pass container packaging path under [Docker/README.md](/home/ubuntu/ChatterBox_S3_Concurrency/Docker/README.md) with a GPU `Dockerfile`, model-prep helper, Compose example, and a mounted `/models` contract for `vLLM + turbo S3`
...
- completed the first working Hydra-free `vLLM + turbo S3` spike on the `RTX A6000` box:
  ...
  - new service-design read:
    ...
    - the first mixed-traffic simulator crash showed the same rule applies there too:
      - disabling prefix caching alone was not enough
