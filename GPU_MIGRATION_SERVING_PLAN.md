...
The service lives in `external/chatterbox/fastapi_vllm_tts_service.py`.
It holds one shared model instance and batches queued requests over a short admission window.
T3 decode runs as a single batched `vllm.generate()` call; S3 finalize runs in parallel
across up to 4 CUDA streams after T3 completes.

### 8a. Checkpoint Paths (Thunder)

| Artifact | Path |
|---|---|
| Base T3 weights | `~/.cache/huggingface/hub/models--ResembleAI--chatterbox/snapshots/05e904af2b5c7f8e482687a9d7336c5c824467d9` |
| Turbo S3 weights (meanflow) | `~/.cache/huggingface/hub/models--ResembleAI--chatterbox-turbo/snapshots/749d1c1a46eb10492095d68fbcf55691ccf137cd` |
| vLLM T3 export | `runs/t3_vllm_export` (generated by step 3 above) |

The turbo S3 checkpoint contains `s3gen_meanflow.safetensors` (2-step ODE, ~5× faster than
the default 10-step S3Gen). Set `TURBO_S3_CHECKPOINT_DIR` so the service loads it automatically.

### 8b. Environment Variables

| Variable | Default | Description |
|---|---|---|
| `VLLM_MODEL_DIR` | _(required)_ | Path to the exported vLLM T3 model package (`runs/t3_vllm_export`) |
| `VLLM_GPU_MEMORY_UTILIZATION` | `0.5` | Fraction of GPU VRAM reserved for the vLLM KV cache. Keep at `0.5` on A6000 to leave room for S3 activations |
| `VLLM_MAX_MODEL_LEN` | `2048` | Maximum token sequence length vLLM allocates for. `2048` is sufficient for all current text lengths |
| `VLLM_ENFORCE_EAGER` | `true` | Set to `false` to enable CUDA graphs — gives ~47% `first_chunk_s` reduction. Safe since the CUDA device-side assert was closed (repetition_penalty=1.0 fast path). **Recommended: `false`** |
| `VLLM_ENABLE_PREFIX_CACHING` | `false` | Keep `false`. Prefix caching is incompatible with embed-only prompts — rows 1..N hit `max_new_tokens` instead of emitting stop tokens |
| `VLLM_DTYPE` | `auto` | vLLM weight dtype. `auto` selects bf16 on Ampere+ |
| `VLLM_TP_SIZE` | `1` | Tensor parallel size. Only change if running multi-GPU |
| `VLLM_PROMPT_BUILDER_DEVICE` | `cpu` | Device for T3 prompt embedding construction. `cpu` offloads this from the GPU to save VRAM |
| `VLLM_EXPORT_DIR` | _(optional)_ | If set, re-exports the T3 model to this dir on startup instead of using `VLLM_MODEL_DIR` |
| `VLLM_EXPORT_COPY` | `false` | If `true`, copies weights instead of symlinking during export |
| `CHECKPOINT_DIR` | _(optional)_ | Path to a Hydra/fine-tuned T3 checkpoint dir (Hydra-free spike: leave unset) |
| `BASE_CHECKPOINT_DIR` | _(optional)_ | Explicit path to base multilingual T3 checkpoint dir (used when `CHECKPOINT_DIR` is a Hydra dir) |
| `TURBO_S3_CHECKPOINT_DIR` | _(optional)_ | Path to the turbo S3 checkpoint dir containing `s3gen_meanflow.safetensors`. If unset, falls back to HF download |
| `DEFAULT_AUDIO_PROMPT_PATH` | _(optional)_ | Default voice prompt WAV file. Requests that omit `audio_prompt` use this |
| `API_DEVICE` | `cuda` | Torch device for model loading |
| `API_BATCH_WINDOW_MS` | `5.0` | Admission window in milliseconds. Requests arriving within this window are batched together |
| `API_MAX_BATCH_SIZE` | `8` | Hard cap on requests per vLLM batch |
...
export VLLM_ENFORCE_EAGER=false        # CUDA graphs — 47% first_chunk_s gain
...
- `LD_LIBRARY_PATH` must be set before starting — vLLM fails with `libcudart.so.12` without it
- `HF_TOKEN` only needed if artifacts are not yet cached locally
- `VLLM_ENFORCE_EAGER=false` is safe since the CUDA device-side assert was closed (repetition_penalty=1.0 activates the `no_penalties` fast path, skipping the stale token_ids_cpu scatter that caused the original crash)
- Ctrl+C triggers graceful drain → engine close (15s timeout) → force-kill vLLM EngineCore children
...
| `/v1/tts` | POST | Synthesise full utterance, return WAV |
...
Transport-streamed WAV (full synthesis then stream):
...
Concurrency-4 smoke with xargs:
...
### 8f. Chunked Streaming (`/v1/tts/stream_chunks`)

The `stream_chunks` endpoint splits text into short phrases (default: target 5 words, max 8 words),
synthesises each chunk independently, and streams each WAV chunk back as it finishes. This gives
significantly lower first-audio latency than waiting for full-utterance synthesis.

Chunking settings in the request body:

| Field | Default | Description |
|---|---|---|
| `chunk_target_words` | `5` | Target words per chunk — chunks aim for this length |
| `chunk_max_words` | `8` | Hard cap per chunk before a forced split. A trailing connective word (و، في، من…) is absorbed to avoid orphaned clause starts |
| `chunk_auto_max_new_tokens_cap` | `128` | Token budget per chunk. Bypasses the dynamic tier — use a flat ceiling so 7-word Arabic chunks (~90 speech tokens) never truncate |

Arabic-specific behaviour:

- Hard punctuation (`.!?؟`) splits immediately at `n≥1` so `جواب؟` becomes its own chunk rather than acting as a mid-utterance EOS signal
- A trailing 1-word fragment is merged into the previous chunk in post-processing
- Connective absorption prevents splits like `…الجملة` / `و` / `التالية`

Use `stream_chunks_client.py` for load testing:
...
python external/chatterbox/stream_chunks_client.py \
  ...
  --save-dir /tmp/tts_chunks_test \
  --summary-json /tmp/tts_chunks_test/summary.json
```

Key metrics to check in the summary:

- `s3_finalize_wait_s (mean)` — should be near `0` with parallel S3 enabled (target: `<0.01s`)
- `first_chunk_s (client mean)` — time to first audio byte at the client (target: `<2.5s` at c=4)
...
After the batched vLLM T3 decode, S3 (mel synthesis + vocoder) runs in parallel across
requests using a `ThreadPoolExecutor(max_workers=4)` with a dedicated `torch.cuda.Stream`
per request. This was the dominant bottleneck before the fix:

- Before: `s3_finalize_wait_s ≈ 0.719s` (sequential, last request waited N × T_s3)
- After: `s3_finalize_wait_s ≈ 0.001s` (all N requests start simultaneously)

The worker cap of 4 is intentional — beyond 4 concurrent S3 streams, peak VRAM pressure
from simultaneous activations risks OOM on a GPU shared with the vLLM KV cache.
Adjust `max_workers=min(n, 4)` in `worker_vllm.py:generate_many()` if you change GPU.
## Common Failures
